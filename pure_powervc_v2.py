...
This driver requires Purity version 6.1.0 or later.
...
from powervc_cinder.volume.discovery_driver import (
    RESTRICTED_METADATA_VDISK_ID_KEY,
)
from powervc_cinder.volume.discovery_driver import (
    RESTRICTED_METADATA_VDISK_NAME_KEY,
)
from powervc_cinder.volume.discovery_driver import (
    RESTRICTED_METADATA_VDISK_pg83NAA_KEY,
)
from powervc_cinder.volume.discovery_driver import (
    RESTRICTED_METADATA_VDISK_UID_KEY,
)
...
class PureFCDriverPowerVC(
    PureFCDriver, discovery_driver.VolumeDiscoveryDriver
):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(PureFCDriverPowerVC, self).__init__(
    ...
    def discover_storage_ports(
        self, details=False, fabric_map=False, all_ports=False
    ):
        available_ports = dict()
        pinfo = {}
        current_array = self._get_current_array()
        ports = list(current_array.get_ports().items)
        hardware = list(current_array.list_hardware().items)
        for port in range(0, len(ports)):
            for count in range(0, len(hardware)):
                if (
                    hardware[count].name == ports[port].name
                    and ports[port].wwn
                    and not ports[port].nqn
                ):
    ...
    def get_volume_info(self, vol_refs, filter_set):
        if vol_refs or filter_set:
            LOG.debug(
                "Filter Set %(filter_set)s " "vol_refs : %(vol_rf)s",
                {"filter_set": filter_set, "vol_rf": vol_refs},
            )
        current_array = self._get_current_array()
        # TODO(Pure): Get managed volumes only?
        pure_volumes = list(current_array.get_volumes().items)
        # This is more efficient than querying individual hosts
        pure_hosts = list(current_array.get_hosts().items)
        # [u'5001500150015000', u'5001500150015001',
        #  u'5001500150015002']
        array_ports = self._get_array_wwns(current_array)
        LOG.debug(
            "Retrieved volumes on FlashArray"
        ...
        for pure_volume in range(0, len(pure_volumes)):
            naa_page83 = PURE_REGISTERED_OUI + pure_volumes[pure_volume].serial
            ...
            all_connections = list(
                current_array.get_connections(
                    volume_names=[pure_volumes[pure_volume].name]
                ).items
            )
            # With single host connections
            # [{'host': {'name': 'c08-01-host1'},
            #   'host_group': {},
            #   'lun': 1,
            #   'protocol_endpoint': {},
            #   'volume': {'id': 'c9fd2447-2fa8-54e9-e9c8-d3d112ccb101',
            #              'name': 'c08-01-host1-BOOT'}}]
            # With shared connections (in host group)
            # [{'host': {'name': 'c08-01-host1'},
            #  'host_group': {'name': 'C08Cluster'},
            #  'lun': 254,
            #  'protocol_endpoint': {},
            # 'volume': {'id': '8a05829b-0200-4163-8fed-213ebbfd0727', 'name': 'C08DS01'}}]
            itl_list = []
            connect_info = {}
            for pure_connection in range(0, len(all_connections)):
                pure_host = next(
                    h
                    for h in pure_hosts
                    if h["name"] == all_connections[pure_connection].host
                )
                # should be only 1
                ...
                connect_object = {
                    ...
                    "target_lun": all_connections[pure_connection].lun,
                    "host": all_connections[pure_connection].host,
                ...
                itl_obj = discovery_driver.ITLObject(
                    pure_host.wwn,
                    array_ports,
                    all_connections[pure_connection].lun,
                    vios_host=all_connections[pure_connection].host.name,
                )
                itl_list.append(itl_obj)
            vol_ret = {
                "name": pure_volumes[pure_volume].name,
                ...
                "pg83NAA": naa_page83,
                "provider_id": pure_volumes[pure_volume].name,
                "metadata": {METADATA_VOLUME_WWN: naa_page83},
                "restricted_metadata": {
                    "vdisk_id": pure_volumes[pure_volume].serial,
                    "vdisk_name": pure_volumes[pure_volume].name,
        ...
        if not volume_type_id:
            ...
            if volume_type:
                ...
                self.db.volume_update(
                    ctxt, volume["id"], {"volume_type_id": volume_type_id}
                )
    ...
    def create_cloned_volume(self, tgt_volume, src_volume):
        ...
        model_update = super(PureFCDriverPowerVC, self).create_cloned_volume(
    ...
    def _get_snap_name(self, snapshot):
        ...
        return "%s.%s" % (
            self._get_vol_name(snapshot.volume),
    ...
    def create_volume(self, volume):
        ...
        """
        model_update = super(PureFCDriverPowerVC, self).create_volume(volume)
    ...
    def _create_restricted_metadata(self, volume_obj, metadata):
        ...
        vdisk = list(
            array.get_volumes(names=[volume_obj["provider_id"]]).items
        )[0]
        PURE_REGISTERED_OUI = "624A9370"
        vdisk_id = PURE_REGISTERED_OUI + vdisk["serial"]
        if vdisk is not None:
            metadata = {
                RESTRICTED_METADATA_VDISK_ID_KEY: vdisk["serial"],
        ...
        return super(PureFCDriverPowerVC, self)._create_restricted_metadata(
            volume_obj, metadata
        )
    ...
    def _model_update(self, model_update, volume):
        ...
        array = self._get_current_array()
        vdisk = list(array.get_volumes(names=[volume["provider_id"]]).items)[0]
        PURE_REGISTERED_OUI = "624A9370"
        vdisk_id = PURE_REGISTERED_OUI + vdisk.serial
